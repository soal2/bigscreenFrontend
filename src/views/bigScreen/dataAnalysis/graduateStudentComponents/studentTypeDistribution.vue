<template>
  <div id="studentTypeDistribution">
    <div class="bg-color-black">
      <div class="d-flex pt-2 pl-2">
        ...
        <div class="d-flex">
          <span class="text mx-2">研究生类型分布</span>
...
export default {
  ...
  data() {
    return {
      ...
      data1Total: 0,
      data2: [
        { name: '全日制硕士', value: 4950 },
        { name: '非全日制硕士', value: 4658 },
        { name: '进修硕士', value: 4658 },
        { name: '留学硕士', value: 1660 },
        { name: '全日制博士', value: 1830 },
        { name: '本科直博', value: 1830 },
        { name: '留学博士', value: 1830 },
        { name: '其他', value: 1830 },
      ],
  ...
  methods: {
    ...
    async initLoadData() {
      await service
        .get('graduateStudent/numDistribution')
        .then(
          res => {
            console.log(res.data)
            this.data1 = res.data
            console.log("load numDistribution success")
          }
        )
        .catch(failResponse => {
      ...
      let rr = null
      await service
        .get('graduateStudent/studentTypeDistribution')
        .then(
          res => {
            console.log(res.data)
            rr = res.data
            console.log("load studentTypeDistribution success")
          }
        )
        .catch(failResponse => {
          console.log("error", failResponse)
      ...
      if (rr) {
        for (let i = 0; i < 8; i++) {
          for (let j = 0; j < 8; j++) {
            //console.log(rr[i].name,this.baseData1[j].name,rr[i].name===this.baseData1[j].name)
    ...
    initCharts() {
        ...
        series: [
          {
            ...
            itemStyle: {
              ...
              color: function (params) {
                const colorList = ['#FAC858','#9665BA', '#EE6666', ]; //'#91CC75',
      ...
      this.chart1.on('click', function (params) {
        if (params.componentType === "series") {
          ...
          _this.$router.push({ name: 'GraduateStudent-StudentTypeDetail', query: { name: params.name, } })
        }
      });
    },
  },
