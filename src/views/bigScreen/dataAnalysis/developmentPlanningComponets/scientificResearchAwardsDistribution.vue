<template>
    <div id="scientificResearchAwardsDistribution">
        <div class="bg-color-black">
            <div class="d-flex pt-2 pl-2">
                ...
                <div class="d-flex">
                    <span class="text mx-2">科学研究情况</span>
                ...
                <div>
                    ...
                    <div style="width: 98%;padding-left: 1%;">
                        <div style="text-align: center;">
                            <span style="color: #D7D8DA; font-size: 10px;line-height: 1;">{{ description }}</span>
                        </div>
                    </div>
                </div>
                <div>
                    ...
                    <div style="width: 98%;padding-left: 1%;">
                        <div style="text-align: center;">
                            <span style="color: #D7D8DA; font-size: 10px;line-height: 1;">{{ description2 }}</span>
                        </div>
                    </div>
                </div>
            </div>
...
export default {
    name: 'ScientificResearchAwardsDistribution',
    data() {
        return {
            chart: null,
            legendClass: ['国家级科研奖励'],
    ...
    methods: {
        ...
        async initLoadData() {
            ...
            for (let i = 0; i < this.legendClass2.length; i++) {
                await service
                    .get('developmentPlanning', { params: { name: this.legendClass2[i] } })
                    .then(
                        res => {
                            console.log(res.data)
                            rr = res.data
                ...
                if (rr) {
                    this.xLabel2 = rr.subject
                    this.yData2.push(rr.value)
                    this.description2 = rr.indicatorDescription
                }
                else {
                    this.yData2.push([])
                }
        ...
        initCharts() {
            ...
            this.chart.on('click', function (params) {
                if (params.componentType === "series") {
                    ...
                    _this.$router.push({ name: 'DevelopmentPlanning-ScientificResearchNationalAwardsDetail', query: { name: params.name, } })
                }
            });
            ...
            this.chart2.on('click', function (params) {
                if (params.componentType === "series") {
                    console.log("点击了" + params.componentType + params.name);
                    _this.$router.push({ name: 'DevelopmentPlanning-ScientificResearchProvincialAwardsDetail', query: { name: params.name, } })
                }
            });
        },
    }
...
#scientificResearchAwardsDistribution {
    ...
    .body-box {
        ...
        .dv-bar-chart {
            padding-top: 8px;
            width: 100%;
            height: 180px; // Adjust based on the available space
            margin-bottom: 1px;
        }
    }
}
